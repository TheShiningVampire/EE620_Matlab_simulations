...
phi_m =phi_sub% (4.05)*q;  %work function of metal
...
%% Input
psi_s=-sign(V_g-V_fb)*0.0001;
pre=(2*eps_sub*q*Vt*abs(N_sub))^0.5;

for i = 1:500
psi0=psi_s;   
Q_s = -sign(V_g-V_fb)*pre*(exp(-psi_s/Vt)+psi_s/Vt-1+(ni^2/(abs(N_sub))^2)*(exp(psi_s/Vt)-psi_s/Vt-1))^0.5;
f= -V_g+V_fb-Q_s/c_ox+psi_s;
df=1/c_ox*(-sign(V_g-V_fb))*(pre)^2*0.5*(-1/Vt*exp(-psi_s/Vt)+1/Vt+(ni^2/abs(N_sub)^2)*(1/Vt*exp(psi_s/Vt)-1/Vt))/Q_s + 1;
psi_s=psi0-f/df;
