...
for i=1:length(V_g)
 ...
 F3 = @(sp) V_fb-(signq*((2*eps_sub*k*T*abs(N_sub))^0.5*((exp(-q*sp/(k*T))+q*sp/(k*T)-1)+(ni^2/(abs(N_sub))^2)*(exp(+q*sp/(k*T))-q*sp/(k*T)-1))^0.5)/c_ox)+sp-V_g(i);
  ...
  F4 = @(sp) V_fb-(signq2*((2*eps_sub*k*T*abs(N_sub))^0.5*((exp(-q*psi_s3(i)/(k*T))+q*psi_s3(i)/(k*T)-1)+psi_s3(i)*(ni^2/((Vt)*(abs(N_sub))^2))*(exp(+q*psi_s2(i)/(k*T))-1))^0.5)/c_ox)+sp-V_g(i)-V_wiggle;
