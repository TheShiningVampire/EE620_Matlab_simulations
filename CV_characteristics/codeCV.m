...
%     end
 ...
 psi_s2(i) = fsolve(F2,-signq2*0.1); % Initial guess, sign taken due to fsolve
 Q2(i,1)=signq2*((2*eps_sub*k*T*abs(N_sub))^0.5*((exp(-q*psi_s2(i)/(k*T))+q*psi_s2(i)/(k*T)-1)+(ni^2/(abs(N_sub))^2)*(exp(+q*psi_s2(i)/(k*T))-q*psi_s2(i)/(k*T)-1))^0.5);
  
 C(i,1)=abs((Q2(i,1))-(Q1(i,1)))/0.05;
