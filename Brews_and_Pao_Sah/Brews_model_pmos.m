% The code has some errors. I could not figure them out
% Solving Brews model for the following NMOSFET
% L = 1 um, Tox = 10 nm, p+ poly-Si gate, Na (choose) so that V_T -> 0.8V
...
Vd = -sign(Nsub) * 0.1;
Id_vec = [];
Vg_vec = -sign(Nsub) * (0:50e-3:2);
dpsi_s = -sign(Nsub) * 1e-3;

for Vg = -sign(Nsub) * (0:50e-3:2)
    QT = @(psi_s) -sign(Nsub) * Cox*(Vg - Vfb - psi_s);
    QD = @(psi_s) (2*eps_si*q*Na*psi_s).^0.5;
    QI = @(psi_s) QT(psi_s) - QD(psi_s);
    dVdpsi_s = @(psi_s) 1 + 2*kT/q * ( -sign(Nsub)*Cox*QT(psi_s) + eps_si*q*Na)./ ((QT(psi_s)).^2 + sign(Nsub)*(QD(psi_s)).^2);
    Vgf = @(psi_s, V) Vfb + psi_s - sign(Nsub)*1/Cox*(2*eps_si*kT*Na)^0.5*( abs( -sign(Nsub)*q*psi_s/kT + ni^2/Na^2*exp(-sign(Nsub)*q*(psi_s-V)/kT) ) ).^0.5;
...
figure(2);
yyaxis left;
semilogy(Vg_vec, abs(Id_vec*1e6));
axes = gca;
