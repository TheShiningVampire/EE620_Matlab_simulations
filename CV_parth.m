...
% Sign of substrate charges in the current mode, since we have signum(psi_s1(i)) in formula of charge
...
%     psi_s1(i) = fsolve(F,-signq*0.1); % Initial guess, sign taken due to fsolve
%     
% else                              % sign(N_sub) > 0
...
for i=1:length(V_g)
 ...
 F1 = @(sp) V_fb-(signq*((2*eps_sub*k*T*abs(N_sub))^0.5*((exp(-q*sp/(k*T))+q*sp/(k*T)-1)+(ni^2/(abs(N_sub))^2)*(exp(+q*sp/(k*T))-q*sp/(k*T)-1))^0.5)/c_ox)+sp-V_g(i);
    psi_s1(i) = fsolve(F1,-signq*0.1); % Initial guess, sign taken due to fsolve
 
  Q1(i,1)=signq*((2*eps_sub*k*T*abs(N_sub))^0.5*((exp(-q*psi_s1(i)/(k*T))+q*psi_s1(i)/(k*T)-1)+(ni^2/(abs(N_sub))^2)*(exp(+q*psi_s1(i)/(k*T))-q*psi_s1(i)/(k*T)-1))^0.5);

  F2 = @(sp) V_fb-(signq*((2*eps_sub*k*T*abs(N_sub))^0.5*((exp(-q*sp/(k*T))+q*sp/(k*T)-1)+(ni^2/(abs(N_sub))^2)*(exp(+q*psi_s1(i)/(k*T))-q*psi_s1(i)/(k*T)-1))^0.5)/c_ox)+-V_g(i)-V_wiggle;
  
  psi_s1(i) = fsolve(F2,-signq2*0.1); % Initial guess, sign taken due to fsolve
  Q2(i,1)=signq2*((2*eps_sub*k*T*abs(N_sub))^0.5*((exp(-q*psi_s1(i)/(k*T))+q*psi_s1(i)/(k*T)-1)+(ni^2/(abs(N_sub))^2)*(exp(+q*psi_s1(i)/(k*T))-q*psi_s1(i)/(k*T)-1))^0.5);
...
for i=1:length(V_g)
 ...
 F3 = @(sp) V_fb-(signq*((2*eps_sub*k*T*abs(N_sub))^0.5*((exp(-q*sp/(k*T))+q*sp/(k*T)-1)+(ni^2/(abs(N_sub))^2)*(exp(+q*sp/(k*T))-q*sp/(k*T)-1))^0.5)/c_ox)+sp-V_g(i);
    psi_s2(i) = fsolve(F3,-signq*0.1); % Initial guess, sign taken due to fsolve
 
  Q3(i,1)=signq*((2*eps_sub*k*T*abs(N_sub))^0.5*((exp(-q*psi_s2(i)/(k*T))+q*psi_s2(i)/(k*T)-1)+(ni^2/(abs(N_sub))^2)*(exp(+q*psi_s2(i)/(k*T))-q*psi_s2(i)/(k*T)-1))^0.5);
    
 F4 = @(sp) V_fb-(signq*((2*eps_sub*k*T*abs(N_sub))^0.5*((exp(-q*sp/(k*T))+q*sp/(k*T)-1)+(ni^2/(abs(N_sub))^2)*(exp(+q*psi_s2(i)/(k*T))-q*psi_s2(i)/(k*T)-1))^0.5)/c_ox)+sp-V_g(i)-V_wiggle;
  
  psi_s3(i) = fsolve(F4,-signq2*0.1); % Initial guess, sign taken due to fsolve
 % Q4(i,1)=signq2*((2*eps_sub*k*T*abs(N_sub))^0.5*((exp(-q*psi_s3(i)/(k*T))+q*psi_s3(i)/(k*T)-1)+psi_s3(i)*(ni^2/((Vt)*(abs(N_sub))^2))*(exp(+q*psi_s2(i)/(k*T))-1))^0.5);
   Q4(i,1)=signq*((2*eps_sub*k*T*abs(N_sub))^0.5*((exp(-q*psi_s2(i)/(k*T))+q*psi_s2(i)/(k*T)-1)+(ni^2/(abs(N_sub))^2)*(exp(+q*psi_s3(i)/(k*T))-q*psi_s3(i)/(k*T)-1))^0.5);
